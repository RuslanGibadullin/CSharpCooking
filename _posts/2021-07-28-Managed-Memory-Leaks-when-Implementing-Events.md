...
и скомпилировать его с включенной отладкой, то локальная переменная для хранения объекта `new StringBuilder("week")` будет создана неявно и сохраняться в активном состоянии до завершения метода `Main`. Это делается для того, чтобы была возможность проверять переменные после того, как они выйдут за пределы области действия, но до завершения метода.
