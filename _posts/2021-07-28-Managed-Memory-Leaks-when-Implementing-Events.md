...
и скомпилировать его в режиме отладки с отключенной оптимизацией, то время жизни объекта `new StringBuilder("week")` расширится до завершения метода `Main`. Это делается для того, чтобы была возможность проверять переменные после того, как они выйдут за пределы области действия, но до завершения метода.
