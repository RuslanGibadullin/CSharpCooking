...
class Test
{
	...
	{
		Client[] clients = Enumerable.Range(0, 1000)[^1]
...
[^1]: Метод `Enumerable.Range(int start, int count)` создает последовательность целых чисел в указанном диапазоне, где `start` – значение первого целого числа для последовательности, а `count` – количество генерируемых последовательных целых чисел.
