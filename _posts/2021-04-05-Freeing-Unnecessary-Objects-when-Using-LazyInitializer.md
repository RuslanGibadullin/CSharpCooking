...
<a name="volatile"/>
> Класс `Lazy<T>` на самом деле реализует микро-оптимизированную версию этого шаблона, которая называется блокированием с двойным контролем. Блокирование с двойным контролем выполняет дополнительное [_volatile-чтение_](/2021/04/04/Read-and-Write-Methods-of-Volatile-Сlass.html), чтобы избежать затрат на получение блокировки, если объект уже инициализирован.
